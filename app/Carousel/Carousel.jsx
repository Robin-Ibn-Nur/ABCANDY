...
export const Carousel = () => {
    ...
    return (
        <div className="max-w-6xl mx-auto h-[540px] md:h-[670px] flex flex-col xl:flex-row items-center overflow-hidden gap-5 lg:gap-10 relative">
            <div className="absolute w-full h-full flex items-center  z-50 px-5">
                {/* arrow left */}
                <button onClick={prevSlider} className="absolute top-50 left-2.5 bg-white rounded-full w-6 h-6 md:w-8 md:h-8">
                ...
                {/* arrow right */}
                <button onClick={nextSlider} className="absolute top-50 right-2.5 bg-white rounded-full w-6 h-6 md:w-8 md:h-8">
            ...
            {/* slider container */}
            <div className="h-[540px] md:h-[670px] w-2/3   ease-linear duration-300 flex items-center"
                style={{ transform: `translateX(-${currentSlider * 50}%)` }}>
                {/* sliders */}
                {sliders.map((slide, inx) => (
                    // <div key={inx}
                    //     className={`${currentSlider === inx ? "h-[240px] sm:h-[310px] md:h-[480px] lg:h-[580px]" : "h-[220px] sm:h-[260px] md:h-[380px] lg:h-[480px] scale-95 opacity-40"} min-w-[50%]  duration-200`}
